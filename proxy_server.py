...
# 🚀 THE MASTER GLOBAL ROUTING INTERCEPTOR (Catches relative JS/CSS/XHR requests)
class GlobalProxyMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        path = request.url.path
        
        # Permit native routes to bypass interception
        if path in ["/proxy", "/docs", "/openapi.json"] or path.startswith("/static"):
            return await call_next(request)
            
        referer = request.headers.get("referer", "")
        
        # If the background asset request was generated by a proxy-loaded page
        if "url=" in referer:
            try:
                # Isolate the Base64 hash parameter from the history context safely
                ...
                # Use standard object parsers to resolve absolute URLs correctly without text string corruption
                parent_url = httpx.URL(decoded_parent)
                full_target_url = str(parent_url.join(path))
                
                raw_query = request.url.query
                if raw_query:
                    full_target_url += f"?{raw_query}"
                    
                print(f"[MIDDLEWARE CATCH] Automatically rerouted broken asset target: {full_target_url}")
                
                # Scramble the path up and forward it back inside the proxy engine loop
...
async def proxy_endpoint(request: Request, url: str = Query(..., description="The BASE64 ENCODED target URL")):
    ...
    # Allow POST methods used by streaming forms and video data pipes
    method = request.method
    async with httpx.AsyncClient(follow_redirects=True) as client:
        try:
            # We match the request type dynamically (GET/POST/HEAD) to follow player mechanics
